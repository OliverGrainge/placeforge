...
def build_parser() -> argparse.ArgumentParser:
    ...
    analyse_parser = subparsers.add_parser(
        "analyse", help="Analyse a training dataset: sample figure + intra-class variation"
    )
    analyse_parser.add_argument("dataset_name", help="Name of the training dataset")
    analyse_parser.add_argument(
        "--num-places", type=int, default=16, help="Number of places to sample (default: 16)"
    )
    analyse_parser.add_argument(
        "--images-per-place", type=int, default=4, help="Images per place to show (default: 4)"
    )
    analyse_parser.add_argument(
        "--embedding-name",
        default=None,
...
def _handle_analyse(args: argparse.Namespace) -> int:
    ...
    print(f"    images/place:     avg={sum(img_counts)/len(img_counts):.1f}  "
          f"min={min(img_counts)}  max={max(img_counts)}")
    print(f"    places/supergroup avg={sum(places_per_sg)/len(places_per_sg):.1f}  "
    ...
    def _summary(series: pd.Series) -> dict:
        return {
            ...
            "p5": float(series.quantile(0.05)), "p25": float(series.quantile(0.25)),
    ...
    def _hist(ax, data, title, xlabel, ylabel, color):
        ax.hist(data, bins=min(60, len(data)), color=color, edgecolor="none", alpha=0.85)
        ax.axvline(data.mean(), color="black", linestyle="--", linewidth=1.0,
                   label=f"mean={data.mean():.3f}")
        ax.axvline(data.median(), color="grey", linestyle=":", linewidth=1.0,
    ...
    if not image_cache.exists:
        ...
        print(f"  Run the datapipeline with ComputeEmbeddingStep, or pass --embedding-name.\n")
        return 0
    ...
    for place_id, group in tqdm(df.groupby("place_id"), desc="Intra-class distances", unit="place"):
        ...
        per_place.append({
            "place_id": int(place_id), "supergroup_id": int(sg_id), "n_images": int(n),
    ...
    intra_summary = {
        "dataset": args.dataset_name, "embedding_name": embedding_name,
    ...
    fig.suptitle(f"Intra-class variation — {args.dataset_name} ({len(place_df):,} places)", fontsize=12)
    _hist(axes[0, 0], place_df["mean_cos_dist"], "Mean cosine distance", "mean pairwise cos-dist", "# places", "#4C72B0")
    _hist(axes[0, 1], place_df["max_cos_dist"],  "Max cosine distance",  "max pairwise cos-dist",  "# places", "#C44E52")
    _hist(axes[0, 2], place_df["min_cos_dist"],  "Min cosine distance",  "min pairwise cos-dist",  "# places", "#55A868")
    _hist(axes[1, 0], place_df["std_cos_dist"],  "Std cosine distance",  "std pairwise cos-dist",  "# places", "#8172B2")
    sorted_vals = np.sort(place_df["mean_cos_dist"].values)
    axes[1, 1].plot(sorted_vals, np.arange(1, len(sorted_vals) + 1) / len(sorted_vals), color="#4C72B0", linewidth=1.5)
    ...
    axes[1, 2].scatter(place_df["n_images"], place_df["mean_cos_dist"], alpha=0.3, s=4, color="#4C72B0", rasterized=True)
    ...
    if n_sg <= 128:
        sg_groups = place_df.groupby("supergroup_id")["mean_cos_dist"].apply(list).sort_index()
        fig2, ax2 = plt.subplots(figsize=(max(8, n_sg * 0.35), 5))
        ax2.boxplot(sg_groups.values, labels=[str(s) for s in sg_groups.index], patch_artist=True,
        ...
        ax2.set_title(f"Intra-class cosine distance by supergroup — {args.dataset_name}", fontsize=10)
    ...
    if not place_cache.exists:
        print(f"  Skipping inter-class analysis: place embeddings not found at {place_cache.cache_dir}\n")
        # Still write the combined stats with only intra
        with open(out_dir / "stats.json", "w") as f:
            json.dump({"intra": {"summary": intra_summary, "per_place": per_place}}, f, indent=2)
        print(f"  Saved stats → {out_dir / 'stats.json'}")
        return 0

    place_emb_index = place_cache.load_index().set_index("id")["row"]
    place_emb_mmap = place_cache.mmap()

    sg_to_places: dict = {}
    for pid in ds.place_ids:
        if pid not in place_emb_index.index:
            continue
    ...
    for sg_id, place_ids in tqdm(sg_to_places.items(), desc="Inter-class distances", unit="supergroup"):
        ...
        if n < 2:
            skipped_single_sg += 1
            continue
        rows = place_emb_index.loc[place_ids].values
    ...
    fig, axes = plt.subplots(2, 3, figsize=(14, 8))
    fig.suptitle(f"Inter-class variation — {args.dataset_name} ({len(sg_df):,} supergroups)", fontsize=12)
    _hist(axes[0, 0], sg_df["mean_cos_dist"], "Mean inter-class cosine distance", "mean pairwise cos-dist", "# supergroups", "#4C72B0")
    _hist(axes[0, 1], sg_df["max_cos_dist"],  "Max cosine distance (most separable)", "max pairwise cos-dist", "# supergroups", "#C44E52")
    _hist(axes[0, 2], sg_df["min_cos_dist"],  "Min cosine distance (most confusable)", "min pairwise cos-dist", "# supergroups", "#55A868")
    _hist(axes[1, 0], sg_df["std_cos_dist"],  "Std cosine distance", "std pairwise cos-dist", "# supergroups", "#8172B2")
    ...
    axes[1, 1].tick_params(labelsize=7)
    axes[1, 1].grid(True, alpha=0.3)
    axes[1, 2].scatter(sg_df["n_places"], sg_df["mean_cos_dist"], alpha=0.5, s=12, color="#4C72B0")
