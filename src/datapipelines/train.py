...
from datapipelines.steps import (
    ...
    ComputeFoVOverlapStep,
    SaveGradedTrainDataset,
)
...
# -----------------------------------------------------------------------------
# SF_XL graded similarity pipeline
# -----------------------------------------------------------------------------

@register_pipeline("sf_xl_graded", category="train")
def build_sf_xl_graded() -> Pipeline:
    name = "sf_xl_graded"
    return Pipeline(
        name,
        steps=[
            ReadTrainImagesStep(data_root=raw_dir() / SF_XL_PATH / "processed" / "train" / "37.74"),
            ComputeFoVOverlapStep(fov_radius=25.0, fov_angle=90.0),
            SaveGradedTrainDataset(name=name),
        ],
    )
