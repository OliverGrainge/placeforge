...
class RemoveSmallPlacesStepTests(unittest.TestCase):
    def test_removes_all_images_from_places_below_threshold(self) -> None:
        dataframe = pd.DataFrame(
            [
                {"image_id": "a1", "place_id": "place_a", "matches": ["a2", "b1", "c1"]},
        ...
        self.assertEqual(filtered["place_id"].tolist(), ["place_a", "place_a", "place_b", "place_b"])
        self.assertEqual(filtered["matches"].tolist(), [["a2", "b1"], ["a1", "b1"], ["a1", "b2"], ["b1"]])
    ...
    def test_context_threshold_can_disable_removal(self) -> None:
        ...
        step = RemoveSmallPlacesStep(min_images_per_place_context_key="min_images_per_place")
